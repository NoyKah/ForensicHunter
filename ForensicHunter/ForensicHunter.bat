...
if "%run_acquisition%"=="1" (
    echo Performing Acquisition...
...
:get_filepath
set /p "filepath=Please enter the path to the C drive you wish to analyze (no need to use double quotes): "
if not exist "%filepath%" (
    echo The path "%filepath%" does not exist. Please try again.
    goto :get_filepath
)

REM Normalize the filepath to ensure it points to the intended directory
for %%D in ("%filepath%\") do set "filepath=%%~fD"
...
echo ---------------------------------------
echo ---------------------------------------
echo All errors have been saved to '%CD%\errors.txt'
	echo ---------------------------------------
...
echo ==========================================
echo.
findstr /s /m /i "%search_term%" "Artifacts\*.*" > Artifacts\%search_term%.txt
echo Results can be found on: Artifacts\%search_term%.txt
echo.
echo ==========================================
echo Searching for Artifacts containing "%search_term%"
echo ==========================================
echo.
